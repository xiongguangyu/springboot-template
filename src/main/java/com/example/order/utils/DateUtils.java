...
public class DateUtils {
	...
	public static final SimpleDateFormat FORMATTIMESTAMPSTR = new SimpleDateFormat("yyyyMMddHHmmss");
	...
	public static final SimpleDateFormat SHORTTIMEFORMAT = new SimpleDateFormat("HHmmss");
	public static final SimpleDateFormat SHORTDAYFORMAT = new SimpleDateFormat("MMdd");
	 ...
	 * 
	 */
	public static String getReqHHmmssDate(Date date) {
	...
	public static String getReqDateyyyyMMddHHmmss(Date date) {
		return FORMATTIMESTAMPSTR.format(date);
	}
	...
	/**
	 * 获取当前格式化日期+时间：yyyy-MM-dd HH:mm:ss.
	 * 
	 * @return
	 */
	 ...
	 * @return
	 */
	public static String getDateByFormatStr(int days, String formatPattern) {
		return getStrFormTime(formatPattern, getDate(days));
	}
	 ...
	 * 规定的时间与当前时间比较大小
	 */
	public static int compareHour(String limitTime) throws ParseException {

		SimpleDateFormat date = new SimpleDateFormat("HH:mm");
		Date limitDate = date.parse(limitTime);
		String str = date.format(new Date());
		Date currentDate = date.parse(str);

		if (limitDate.getTime() - currentDate.getTime() > 0) {
			return 1;
	 ...
	 * @param count 前*天
	 * @param pattern 日期表达式
	 * @param isCountToday 是否算上今日
	 * @return
	 */
	public static List<String> getDaysByPattern(int count,String pattern,boolean isCountToday) {
		DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern(pattern);
		LocalDate startTime1 = LocalDate.now().minusDays(count);
		LocalDate nowDate = LocalDate.now();
		int n = 0 ;
		if(isCountToday){
			n = 1;
		}
		List<String> dates = Stream.iterate(startTime1, date -> date.plusDays(1)).map(dateTimeFormatter::format)
				.limit(ChronoUnit.DAYS.between(startTime1, nowDate)+n)
				.collect(Collectors.toList());
		return dates;
	}
	public static List<String> getMinsByPattern(int count,String pattern){
		DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern(pattern);
		LocalTime startTime = LocalTime.MIN;
		LocalTime nowTime = LocalTime.now();
		List<String> dates = Stream.iterate(0, (i) -> i+count)
				.map(startTime::plusMinutes)
				.filter((nowTime::isAfter))
				.map(dateTimeFormatter::format)
				.limit(ChronoUnit.MINUTES.between(startTime, nowTime)/count)
				.collect(Collectors.toList());
		return dates;
	}
...
//		String a = "07:30";
//		SimpleDateFormat date = new SimpleDateFormat("HH:mm");
//		Date parse = date.parse(a);
//		System.out.println(parse);
//
//		String str = date.format(new Date());
//		System.out.println(str);
//		Date parse2 = date.parse(str);
...
//		}
		...
		System.out.println(getMinsByPattern(5,"HH-mm"));
